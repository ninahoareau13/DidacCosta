...
%load('BWR_2015_salinity_only_good_data.mat')
load('BWR_2015_filtered.mat')
qc = data.SSPS_QC;
dd = find(qc==1);

lat_2015=data.LATX(dd);
lon_2015 = data.LONX(dd);
sal_2015 = data.SSPS(dd);

data_2015=datenum(datetime(data.YEAR(dd),data.MNTH(dd),data.DAYX(dd),data.hh(dd),data.mi(dd),data.ss(dd)));

coloc_sss_nonan = double(sal_2015);
coloc_lon_nonan = double(lon_2015);
coloc_lat_nonan = double(lat_2015);
coloc_time_nonan = double(data_2015);
...
m_pcolor(bec.lon,bec.lat,bec.sss(:,:,80)); shading flat; % 30 January 2015
hold on
%situ
a=1:1:length(coloc_lon_nonan);
...
legend('2015','SMOS SSS','in-situ SSS','Subantarctic Front', 'Subtropical Front','Location','westoutside')
title('')
